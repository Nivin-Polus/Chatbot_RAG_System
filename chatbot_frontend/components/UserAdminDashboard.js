...
const UserAdminDashboard = ({ onLogout }) => {
  ...
  const [websiteMappingLoading, setWebsiteMappingLoading] = useState(false);
  const [websiteMappingError, setWebsiteMappingError] = useState(null);
    ...
    const fetchData = async () => {
      ...
      try {
        if (activeTab === 'files') {
          ...
          const effectiveCollectionId = selectedCollectionId || collectionsData[0]?.collection_id;
          if (effectiveCollectionId) {
            await loadWebsiteMappings(effectiveCollectionId);
          } else {
            setWebsiteMappings([]);
          }
        }
  ...
  useEffect(() => {
    if (activeTab === 'integrations' && selectedCollectionId) {
      loadWebsiteMappings(selectedCollectionId);
    } else if (activeTab === 'integrations' && !selectedCollectionId) {
      setWebsiteMappings([]);
    }
  }, [activeTab, selectedCollectionId]);
  ...
  const loadWebsiteMappings = async (collectionIdParam) => {
    ...
    try {
      setWebsiteMappingLoading(true);
      setWebsiteMappingError(null);
      const response = await api.get(`/collections/${targetCollectionId}/websites`);
      setWebsiteMappings(response.data);
      return response.data;
    } catch (error) {
      const detail = error.response?.data?.detail || error.message;
      setWebsiteMappingError(detail);
      return [];
    } finally {
      setWebsiteMappingLoading(false);
    }
  };

  const handleAddWebsiteUrl = async (e) => {
    e.preventDefault();
    ...
    try {
      await api.post(`/collections/${selectedCollectionId}/websites`, { url });
      setNewWebsiteUrl('');
      await loadWebsiteMappings(selectedCollectionId);
      alert('Website URL added successfully!');
    } catch (error) {
      alert('Error adding website URL: ' + (error.response?.data?.detail || error.message));
    }
  };

  const handleDeleteWebsiteUrl = async (mappingId) => {
    if (!selectedCollectionId) return;

    if (!window.confirm('Remove this website mapping?')) {
      return;
    }

    try {
      await api.delete(`/collections/${selectedCollectionId}/websites/${mappingId}`);
      await loadWebsiteMappings(selectedCollectionId);
      alert('Website URL removed successfully!');
    } catch (error) {
      alert('Error removing website URL: ' + (error.response?.data?.detail || error.message));
    }
  };
  ...
  return (
    <div className="user-admin-dashboard">
      ...
      <div className="dashboard-tabs">
        ...
        <TabButton id="integrations" label="🔗 Website Mapping" isActive={activeTab === 'integrations'} onClick={setActiveTab} />
      </div>
            ...
            <div className="section">
              ...
              {!selectedCollectionId ? (
                <div className="empty-state-modern">
                  ...
                  <p>Create or select a collection to manage website mapping.</p>
                </div>
              ) : (
                <>
                  <form className="create-form" onSubmit={handleAddWebsiteUrl}>
                    <div className="form-row">
                      <input
                        type="url"
                        ...
                        required
                      />
                      <button type="submit" className="create-btn">Add URL</button>
                    </div>
                  </form>

                  {websiteMappingError && (
                    <div className="alert error">{websiteMappingError}</div>
                  )}

                  <div className="section">
                    <h3>Mapped URLs</h3>
                    {websiteMappingLoading ? (
                      <div className="loading">Loading website mappings...</div>
                    ) : websiteMappings.length === 0 ? (
                      <div className="empty-state-modern">
                        <div className="empty-illustration">🔍</div>
                        <h3>No website URLs</h3>
                        <p>Add a URL above to link this collection to an external website or plugin.</p>
                      </div>
                    ) : (
                      <div className="mappings-list">
                        {websiteMappings.map(mapping => (
                          <div key={mapping.id} className="mapping-item">
                            <div className="mapping-details">
                              <strong>{mapping.url}</strong>
                              <span className="mapping-subtext">Normalized: {mapping.normalized_url}</span>
                              {mapping.created_at && (
                                <span className="mapping-subtext">Added: {new Date(mapping.created_at).toLocaleString()}</span>
                              )}
                            </div>
                            <button
                              type="button"
                              className="delete-btn"
                              onClick={() => handleDeleteWebsiteUrl(mapping.id)}
                            >
                              Remove
                            </button>
                          </div>
